...
from dataset import NPYDatasetv2Norm
import wandb
import numpy as np
import sys
...
def sample_test(path1,path2,model,similarity_config,device):
    ...
    array1_flat = (array1_flat - min_vals) / (max_vals - min_vals + 1e-8)  # avoid /0
    array1_flat = np.clip(array1_flat, 0.0, 1.0)  # keep clean range
    ...
    # Apply Min–Max normalization
    array2_flat = (array2_flat - min_vals) / (max_vals - min_vals + 1e-8)  # avoid /0
    array2_flat = np.clip(array2_flat, 0.0, 1.0)  # keep clean range
...
def main(config_path, run_time=0):
    ...
    # Load dataset
    dataset1 = NPYDatasetv2Norm(
        csv_path=config.CSV_PATH
    )
    # dataset2 = NPYDatasetv2Norm(
    #     csv_path=config.CSV_PATH2
    # )
    testset = NPYDatasetv2Norm(
