...
def main(config_path):
    ...
    evaluate_collect_outputs(
        ...
        npy_path=os.path.join(eval_save_dir, f"vec_{config.RUN_NAME}_{last_epoch}_L1.npy"),
        csv_path=os.path.join(eval_save_dir, f"meta_{config.RUN_NAME}_{last_epoch}_L1.csv"),
        to_float32=True
    )
    evaluate_collect_outputs(
        model=model,
