...
if __name__ == '__main__':
    ...
    run_name_short = config.RUN_NAME_SHORT

    vector_storage_path = config.READ_BASE_PATH

    save_path = f"{config.WRITE_BASE_PATH}/{run_name_short}/"
    ...
    for condition in [""]:
        for run_time in range(config.RUN_TIMES_START, config.RUN_TIMES_END): 
            for epoch in range(10, 160, 10):
                ...
                # Load data
                # -------------------------------
                run_name_path = os.path.join(".", vector_storage_path, condition+run_name, f"{run_time}")
                ...
                vecs = np.vstack((vec_L2))
                metas = pd.concat([meta_L2], axis=0, ignore_index=True)
                ...
                print(f"[DONE] Epoch {epoch}, Run {run_time}, Condition '{condition}'")
