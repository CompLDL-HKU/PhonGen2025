"""
We export all checkpoints' results
This one specifically used for unequal as equal setting, where we want to evaluate on the same test set for both train sets. 
"""
# train.py
import argparse
...
def main(config_path, run_name=None, write_name=None, run_time=0):
    ...
    if write_name is not None:
        config.WRITE_NAME = write_name
    else:
        raise ValueError("Please provide write_name argument.")
    ...
    eval_save_dir_hyper = os.path.join(config.WRITE_BASE_PATH, 'eval_outputs', config.WRITE_NAME, f'{run_time}')
    ...
    if checkpoint_files:
        for check_point_file in checkpoint_files: 
            ...
            model.load_state_dict(torch.load(this_checkpoint))
...
if __name__ == '__main__':
    ...
    for run_name, write_name in zip(pre_load_config.RUN_NAMES, pre_load_config.WRITE_NAMES):
        for run_time in range(pre_load_config.RUN_TIMES_START, pre_load_config.RUN_TIMES_END):
            print("Collecting: ", run_name, run_time)
            main(args.config, run_name=run_name, write_name=write_name, run_time=run_time)